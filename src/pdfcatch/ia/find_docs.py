...
from pdfcatch.ia.catalog_store import CATALOG_INDEX_PATH, get_tag_entry, normalize_tag
from pdfcatch.ia.compare_with_paradigm import _load_sentence_transformer_gpu, _load_paradigm
...
def main(argv: list[str] | None = None) -> int:
    ...
    ap.add_argument(
        "--out",
        default=None,
        help="JSON de saída (padrão: io/out/find_docs_<tag>_<ts>.json)",
    )
    args = ap.parse_args(argv)

    norm_tag = normalize_tag(str(args.tag))
    out = Path(args.out) if args.out else Path("io/out") / f"find_docs_{norm_tag}_{_now_compact()}.json"
    return run(
        tag=norm_tag,
