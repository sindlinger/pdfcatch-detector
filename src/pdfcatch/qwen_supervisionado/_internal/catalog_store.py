...
from pdfcatch.qwen_supervisionado._internal.paths import QWEN_CATALOG_ROOT, ensure_catalog_dir, bootstrap_layout

CATALOG_INDEX_PATH = QWEN_CATALOG_ROOT / "catalog_index.json"
...
def normalize_tag(tag: str) -> str:
    ...
    if not s:
        raise ValueError("tag vazia/invalida")
    return s


def catalog_dir_for_tag(tag: str) -> Path:
    return ensure_catalog_dir(QWEN_CATALOG_ROOT / normalize_tag(tag))
...
def load_index(path: Path = CATALOG_INDEX_PATH) -> dict[str, Any]:
    ...
    if not isinstance(obj, dict):
        raise ValueError(f"catalog index invalido: {path}")
    obj.setdefault("schema", "catalog_index.v1")
    obj.setdefault("updated_at_utc", now_utc_iso())
    obj.setdefault("scope", "qwen_isolated")
    obj.setdefault("tags", {})
    if not isinstance(obj["tags"], dict):
        raise ValueError(f"campo tags invalido em: {path}")
    return obj
